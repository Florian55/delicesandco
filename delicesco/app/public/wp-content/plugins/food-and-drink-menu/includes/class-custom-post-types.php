...
class fdmCustomPostTypes {
					...
					<?php foreach ( $fields as $field ) {?>
						...
						<div class='fdm-menu-item-custom-field-input'>
							...
							<?php elseif ( $field->type == 'checkbox' ) : ?>
								<?php $field_values = explode(",", $field->values); ?>
								<?php foreach ( $field_values as $value ) { ?>
									<input type='checkbox' name='<?php echo $field->slug; ?>[]' value='<?php echo sanitize_title($value); ?>' <?php echo ( isset( $values[$field->slug] ) and in_array( sanitize_title($value), $values[$field->slug] ) ) ? 'checked="checked"' : ''; ?> /><?php echo $value;?><br />
								<?php } ?>
							<?php endif; ?>
						</div>
					<?php } ?>
	...
	public function show_menu_item_ordering_options() {
		...
		<table class="fdm-menu-item-ordering-options">
			...
			<?php } ?>
				...
				<tr>
					<td colspan='4' class='fdm-menu-item-add-ordering-option' data-nextkey='<?php echo $max_key + 1; ?>'><?php _e( 'Add', 'food-and-drink-menu' ); ?></td>
				</tr>
			</tbody>
	...
	public function add_section_image() { ?>
		<div class='form-field fdm-menu-section-image'>
			...
			<input id="fdm_menu_section_image_button" class="button" type="button" value="Upload Image" />
	...
	public function edit_section_image( $tag ) { 
		...
		<tr class='form-field fdm-edit-menu-section-image'>
			...
			<td>
				<img class='fdm-edit-menu-section-image-preview' src='<?php echo ($image_id ? wp_get_attachment_url($image_id) : ''); ?>' />
				<input id="fdm_menu_section_image" type="text" size="36" name="fdm_menu_section_image" value="<?php echo ($image_id ? wp_get_attachment_url($image_id) : ''); ?>" />
				<input id="fdm_menu_section_image_button" class="button" type="button" value="Upload Image" />
			<p><?php _e("Upload an image of the for this section:", 'food-and-drink-menu'); ?></p>
		</div>
	<?php }
	...
	public function save_meta( $post_id ) {
		...
		if ( FDM_MENUITEM_POST_TYPE == $post->post_type ) {
			...
			if ( isset( $_POST['ordering_option'] ) ) { 
				update_post_meta( $post->ID, '_fdm_ordering_options', sanitize_text_field( $_POST['ordering_option'] ) );
			}
